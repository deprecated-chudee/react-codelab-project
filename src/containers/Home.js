...
class Home extends Component {
    ...
    componentDidMount() {
        ...
        $(window).scroll(() => {
            // WHEN HEIGHT UNDER SCROLLBUTTON IS LESS THEN 250
            ...
            () => {
                if(this.props.postStatus.status === "SUCCESS") {
                    ...
                    switch(this.props.postStatus.error) {
                        case 1:
                            $toastContents = $('<span stype="color: #FFB4BA">You are not logged in</span>')
                            Materialize.toast($toastContents)
                            setTimeout(() => { location.reload(false) }, 2000)
                            break
                        case 2: 
                            $toastContents = $('<span stype="color: #FFB4BA">Please write something</span>')
                            Materialize.toast($toastContents)
                            break
                        default: 
                            $toastContents = $('<span stype="color: #FFB4BA">Something Broke</span>')
                            Materialize.toast($toastContents)
                            break
                    }
                }
    ...
    loadNewMemo() {
        ...
        // IF PAGE EMPTY, DO THE INITIAL LOADING
    ...
    loadOldMemo() {
        // CALCEL IF USER IS READING THE LAST PAGE
        if(this.props.isLast) {
            return new Promise((resolve, reject) => {
                resolve()
            })
        ...
        this.props.memoRemoveRequest(id, index).then(
            () => {
                if(this.props.removeStatus.status === 'SUCCESS') {
                    console.log(this.props.removeStatus)
...
const mapStateToProps = (state) => {
    return {
        ...
        editStatus: state.memo.edit.status,
